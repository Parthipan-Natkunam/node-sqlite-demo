...
const {INTERNAL_SERVER_ERROR,NOT_FOUND,PRODUCT_SUCCESS} = require("../utils/responseMessages");
...
const products = {
    ...
    updateItemById: async(request,response,next) => {
      ...
      const {name,description,price,available_units} = request.body;
      try{
        await database.updateByID(id,[name,description,price,available_units]);
        response.status(200).json({"message":PRODUCT_SUCCESS.PRODUCT_UPDATED_SUCCESSFULLY});
      }catch(error){
        if(error && error.customErrorCode === 404){
          response.status(error.customErrorCode).json({"message": NOT_FOUND});
        }else{
