...
function cleanProductRequestData (request,response,next){
  ...
  const cleansedData = stripTagsFromStrings([name,description,price,available_units]);
  request.body = {...request.body,name:cleansedData[0],description:cleansedData[1],price:cleansedData[2],available_units:cleansedData[3]};
  next();
}
